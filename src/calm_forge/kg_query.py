...
def kg_query(
    ...
    Returns a list of result dicts:
      {"node": <node>, "related": [...], "namespace": <ns>}
    """
    if namespace == "*":
        return _kg_query_all_namespaces(kg_dir, node_type, where, follow)
    from .kg_namespace import resolve_kg_dir
    effective_dir = resolve_kg_dir(kg_dir, namespace)
    return _kg_query_single(effective_dir, node_type, where, follow, namespace or "")
...
def _kg_query_single(
    ...
    for node in matched:
        ...
        results.append({"node": node, "related": related, "namespace": namespace})
...
def _kg_query_all_namespaces(
    ...
    for ns_name, ns_dir in list_namespaces(kg_dir):
        results.extend(_kg_query_single(ns_dir, node_type, where, follow, ns_name))
    return results
